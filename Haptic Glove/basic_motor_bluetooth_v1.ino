...
int incomingHaptic[4];		// Todo: allow for incoming data from 4 sensors vs 1
...
void setup() {
	...
	digitalWrite(startMotor1, HIGH);	// Enable motors (set EN pin on DRVs to high)
}

void receive_haptic() {  
	int i = 0;
	//if (Serial.available() > 1) {		// why not for loop?
	while (Serial.available()) {
			incomingHaptic[i] = Serial.read();		// Read the incoming byte
			i++;
	} 
}
...
void loop() {
	receive_haptic();							// Check to see if any bt data is avail
